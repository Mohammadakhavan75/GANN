...
def evaluation(population, x, y):
    ...
    evaluation_result = []
    for p in population:
        evaluation_result.append(p.evaluate(x, y, verbose=0, batch_size=60000)[1])
    return evaluation_result


# Checked
def probability(fitness):
    return [x / np.sum(fitness) for x in fitness]
...
def child_val(child, x, y, dicts, population ):
    ...
    child_fit = child.evaluate(x, y, verbose=0, batch_size=60000)[1]
    child_prob = child_fit / (child_fit + np.sum(fit_result) - np.min(fit_result))
...
while True:
    ...
    for p in list(dicts.keys()):
        dicts[p][1] = dicts[p][0] / np.sum([dicts[p][0] for p in list(dicts.keys())])
